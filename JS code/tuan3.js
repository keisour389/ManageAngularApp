    ...
    loadData: function () {
        ...
        if (this.totalPage > 0) {
            ...
            for (let index = start; index < end; index++) {
                dataField.innerHTML += `<tr class="row" id="idx-${index}"></tr>`;
                this.renderItem(index, this.dataShow[index]);
            }
        }
        this.pagination(start, end);

    },
    renderItem: function (index, item) {
        let dataRow = document.getElementById(`idx-${index}`);
        let cbIsMarried = `<input type="checkbox" id="cb-married-${index}" name="cb-married" value="${item.Married}" disabled`;
        if (item.Married) {
            cbIsMarried += ` checked/>`;
        } else {
            cbIsMarried += `/>`;
        ...
        dataRow.innerHTML = `
                  <td class="col idx-${index} id-name" id="name${index}" onclick="updatePerson(${index})">
                      ${item.Name}
                  </td>
                  <td class="col idx-${index} id-age">
                      ${item.Age}
                  </td>
                  <td class="col idx-${index} id-address" id="address${index}" onclick="updatePerson(${index})">
                      ${item.Address}
                  </td>
                  <td class="col idx-${index} id-country" onclick="updatePerson(${index})">
                      ${this.getCountryData(item.Country)}
                  </td>
                  <td class="col idx-${index} id-ismarried" onclick="updatePerson(${index})">
                      ${cbIsMarried}
                  </td>
                  <td class="col idx-${index} id-action">
                    <span onclick="saveData(${index})">&#9997;</span>
                    <span>&#10060;</span>
                  </td>`;
    },
    getCountryData: function (id) {
        for (key of this.countryData) {
            if (key.Id == id)
                return key.Name;
        }
    },
...
function updatePerson(index) {
    if(updateFlag){
        ...
        nameInput.setAttribute("id", `nameField${index}`);
        nameInput.value = name.innerText;
    
        let addressInput = document.createElement('input');
        addressInput.setAttribute("type", "text");
        addressInput.setAttribute("id", `addressField${index}`);
        addressInput.value = address.innerText;
    
        address.innerHTML = nameInput.innerHTML;
        name.innerHTML = addressInput.innerHTML;
...
function saveData(index){
    ...
    let nameField = document.getElementById(`nameField${index}`);
    let addressField = document.getElementById(`addressField${index}`);

    let nameTd = document.createElement('td');
    nameTd.innerText = nameField.value;
    //nameTd.setAttribute("id", `name${index}`);

    let addressTd = document.createElement('td');
    addressTd.innerText = addressField.value;
    //addressTd.setAttribute("id", `address${index}`);

    address.innerHTML = addressTd.innerHTML;
    name.innerHTML = nameTd.innerHTML;

    // name.appendChild(nameTd);
    // address.appendChild(addressTd);
